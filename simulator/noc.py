...
from utils.definitions import Slice, Direction, Message, ceil
...
class Router:
    ...
    def router_fail(self, times):
        for dir in Direction:
            self.links[dir]['in'].change_delay(times)
            self.links[dir]['out'].change_delay(times)


    def router_recover(self, times):
        for dir in Direction:
            self.links[dir]['in'].recover_delay(times)
            self.links[dir]['out'].recover_delay(times)


    def route(self, msg: Message, next_dir, next_router):
        yield self.links[next_dir]['out'].put(msg)

    def route_core(self, msg):
        yield self.links['core']['out'].put(msg)
        ...
        for row in range(self.x):
            for col in range(self.y):
                ...
                if row < self.x - 1:
                    ...
                    link2.bind((row,col), (row+1,col), router_id, east_router_id, True)
                    link1.bind((row+1,col), (row,col), east_router_id, router_id, True)

                    self.routers[router_id].bind_link(Direction.EAST, link1, link2)
                    self.routers[east_router_id].bind_link(Direction.WEST, link2, link1)
                ...
                if col > 0:
                    ...
                    link2.bind((row,col), (row,col-1), router_id, south_router_id, True)
                    link1.bind((row,col-1), (row,col), south_router_id, router_id, True)

                    self.routers[router_id].bind_link(Direction.SOUTH, link1, link2)
                    self.routers[south_router_id].bind_link(Direction.NORTH, link2, link1)
    ...
    def build_connection_torus(self):
        ...
        for row in range(self.x):
            for col in range(self.y):
                ...
                link2.bind((row, col), (east_row, col), router_id, east_router_id, True)
                link1.bind((east_row, col), (row, col), east_router_id, router_id, True)
                ...
                link2.bind((row, col), (row, south_col), router_id, south_router_id, True)
                link1.bind((row, south_col), (row, col), south_router_id, router_id, True)
        ...
        for k in range(num_layers):
            ...
            if num_nodes > 1:
                for i in range(num_nodes):
                    ...
                    self.routers[curr_id].bind_link(Direction.EAST, link_cw, link_ccw)
                    self.routers[next_id].bind_link(Direction.WEST, link_ccw, link_cw)
            ...
            if k < num_layers - 1:
                ...
                for (outer_pos, inner_pos) in corners:
                    ...
                    self.routers[outer_id].bind_link(Direction.SOUTH, link_inward, link_outward)
                    self.routers[inner_id].bind_link(Direction.NORTH, link_outward, link_inward)
    ...
    def build_connection_dragonfly(self):
        ...
        for g_idx, nodes in enumerate(all_groups):
            ...
            for i in range(group_size):
                for j in range(i + 1, group_size):
                    ...
                    link1.bind(coord_u, coord_v, u_id, v_id, True)
                    link2.bind(coord_v, coord_u, v_id, u_id, True)
        ...
        for g_src in range(num_groups):
            for g_dst in range(g_src + 1, num_groups):
                ...
                link_out.bind(coord_src, coord_dst, src_id, dst_id, True)
                link_in.bind(coord_dst, coord_src, dst_id, src_id, True)
        ...
        # # for 4x4 architecture
        # GROUP_SIZE = 4
        # NUM_GROUPS = 4
        
        # # WEST port as global link
        # local_ports = [Direction.NORTH, Direction.SOUTH, Direction.EAST]

        # # intra-group clique
        # for g in range(NUM_GROUPS):
        #     base_id = g * GROUP_SIZE
        ...
        #             u_neighbors = sorted([x for x in range(GROUP_SIZE) if x != u_local])
        #             v_neighbors = sorted([x for x in range(GROUP_SIZE) if x != v_local])
                    
        #             u_port_idx = u_neighbors.index(v_local)
        #             v_port_idx = v_neighbors.index(u_local)
        ...
        #             coord_u = self.routers[u_global].to_xy(u_global)
        #             coord_v = self.routers[v_global].to_xy(v_global)

        #             link1.bind(coord_u, coord_v, u_global, v_global, True)
        #             link2.bind(coord_v, coord_u, v_global, u_global, True)

        #             self.routers[u_global].bind_link(dir_u, link2, link1)
        #             self.routers[v_global].bind_link(dir_v, link1, link2)
        ...
        #         if g < r:
        #             # Group g, Node r
        #             src_id = g * GROUP_SIZE + r
        #             # Group r, Node g
        #             dst_id = r * GROUP_SIZE + g
        ...
        #             self.routers[src_id].bind_link(Direction.WEST, link_in, link_out)
        #             self.routers[dst_id].bind_link(Direction.WEST, link_out, link_in)
