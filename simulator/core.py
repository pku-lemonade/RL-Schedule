...
class Scheduler:
    ...
    def update(self, task: Task):
        ...
        for son_id in task.successors:
            ...
            # check whether the following operator are ready 
            self.mapper.update(src_node=cur_node, dst_node=son_node)

            if son_node.ready == True and son_node.executed == False:
                son_node.executed = True
                if son_node.core_id == self.id:
                    logger.debug(f"    push task {son_node.index} into Core {self.id}")
                    heapq.heappush(self.ready_tasks, Task(node=son_node))
                else:
                    logger.debug(f"    push task {son_node.index} into Core {son_node.core_id}")
                    heapq.heappush(self.cores[son_node.core_id].scheduler.ready_tasks, Task(node=son_node))
...
class Core:
    ...
    def execute(self):
        ...
        while True:
            ...
            result = yield simpy.events.AnyOf(self.env, self.running_event)
