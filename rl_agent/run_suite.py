...
from rl_agent.evaluate_continuous import rollout_continuous
...
def _apply_progress_record(suite_summary: Dict, record: Dict):
    ...
    if record["status"] == "ok":
        _accumulate_stats(suite_summary["method"]["overall"], record["summary"])
        _accumulate_stats(suite_summary["method"]["per_pattern"][pattern], record["summary"])
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="Run continuous-evaluation suites for the trained RL method and emit aggregated summaries."
    )
    ...
    parser.add_argument("--method-name", default="rl_with_fail_prob", help="Method name written into summaries")
    ...
    parser.add_argument("--jobs", type=int, default=min(8, os.cpu_count() or 1), help="Number of fail cases to run in parallel")
    ...
    parser.add_argument(
        "--output-dir",
        default="../data/rl-result/rl_with_fail_prob",
