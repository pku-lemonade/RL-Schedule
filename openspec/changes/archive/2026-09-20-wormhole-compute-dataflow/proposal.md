...
- **RL/embedding:** Keep detailed legacy 4-D hardware features and current observation/action/checkpoint contracts. Inspection of the root RL implementation confirms `MultiDiscrete([num_layers, num_cores, num_cores, num_ops])` (layer, source, destination, operation), with `replace/split/shift/remove`; this corrects the earlier planning shorthand without changing that implementation. No Wormhole remapping/training path is enabled. Router count is not a substitute for enabled worker count.
