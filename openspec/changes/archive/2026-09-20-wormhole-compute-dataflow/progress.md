...
## Part 6 — explicit FC import and legacy consumer boundaries (2026-09-17)

Tasks 6.1–6.5 add `compute_adapters.import_legacy_fc` and the strict `legacy_fc_chain_v1` sidecar. The adapter snapshots every DFG node, slice, edge and runtime flag into immutable records. Explicit core-to-physical-worker bindings, disjoint complete LOAD_FEAT/LOAD_WGT → FC → STORE chains and a complete normalized workload must cover every node and job exactly once. Extra edges, fanout/reuse, partial reductions, cross-core chains, unsupported operations and missing metadata reject before runtime allocation. The sidecar explicitly asserts `explicit_element_count_reshape_v1`: slice extents must match between load/FC/store and their element counts must match the normalized tensors; the importer does not infer tensor axes, bytes, physical mapping or layout conversion. Readiness flags are recorded as provenance, not replayed.

`ComputeWorkload.legacy_import` carries generated immutable DFG and sidecar content digests. The graph/profile source digest retains its existing meaning; import provenance changes configuration and execution identities. Imported local and remote FC executions match equivalent direct workloads in all operational evidence, including complete nested memory traces. Serialization before/after import and execution proves mapper state is unchanged; later mapper mutation cannot change the compiled plan.

The old detailed `Task.execute` FC branch silently did no work. It now raises `NotImplementedError` with the explicit adapter/runtime path. Other legacy branches are unchanged. The existing actual `predictor.predict.detect` and `embedding.hw_encoder.build_hardware_graph` public guards already rejected incompatible input before optional model imports; new tests exercise workload/result documents, plan objects and stage rows, including a subprocess that forbids Torch/PyG imports. No additional predictor implementation or RL path was enabled.

### Validation

- `.venv/bin/python -m unittest simulator_detailed.tests.test_compute_adapters simulator_detailed.tests.test_memory_adapters simulator_detailed.tests.test_topology_consumers`: **27 discovered, 26 passed, 1 optional skip**.
- `.venv/bin/python -m unittest discover -s simulator_detailed/tests`: **307 discovered, 306 passed, 1 optional Torch/PyG skip**. Existing custom-mesh timing/fault, DMA, scratchpad and LOAD/STORE/CONV/POOL/SEND/RECV fixtures passed.
- Strict Pyright: **0 errors / 0 warnings**, including the new adapter. Scoped Ruff and `git diff --check` passed. Strict OpenSpec validation passed.
- Separate root Darknet19 smoke used real `NetworkMapper(parse_mapping(...)).gen_dfg()`, root `Arch.execute()` and `process_events(..., 11, ...)`, with `THERMAL_TIMING_LOG` and JSON trace output inside a `TemporaryDirectory`. **37,888/37,888 nodes finished; 16 cores, 48 directed links, 11 trace windows; trace JSON roundtrip passed**. Observed makespan 19,188,203 cycles is diagnostic, not a new timing fixture.
- Additional root `tests.test_local_remap` probes: reference-DFG equivalence and transactional replacement passed. Two old tests failed: `test_internal_view_layer5` accesses nonexistent `LayerView.active_cores`; `test_safe_layer5_sequence_drains` assumes core 1 is active in layer 5, which current mapping rejects. `git diff be46290 -- utils simulator configs rl_agent tests workloads` is empty: these root inputs, implementation and tests are unchanged by this part. These pre-existing stale checks were recorded, not edited or represented as passing. The independently executed current root mapping/simulation/trace path above passed.
