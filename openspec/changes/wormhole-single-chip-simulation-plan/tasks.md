...
These are umbrella milestones, completed through independently scoped child tasks. Do not apply this file as one implementation change. Each child must define module-scoped tasks small enough for one session, with its own verification commands. All boxes began unchecked because planning artifact completion does not implement these milestones. The 2026-09-20 [evidence audit](verification.md) verifies the seven delivered children and the [specification reconciliation](reconciliation.md) closes the final milestone: **38/38 complete**. Child exit reconciliation means reviewed delta contracts and delivery evidence, not historical main-spec synchronization. Task 8.3 now records seven canonical main specifications containing all 100 distinct parent/child requirement IDs and 178 scenarios. Exact requirement/scenario/code/test references are in [evidence-map.json](evidence-map.json), and the canonical file/input identities are in [spec-sync-identities.json](spec-sync-identities.json). No change has been archived or pushed.
