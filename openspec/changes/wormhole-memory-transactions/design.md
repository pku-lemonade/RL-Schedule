...
Part 7 implements this as `ScratchpadCapacityAdapter.reserve(MemoryBuffer)` and `release(BufferHandle)`. Both return plain completion events; accepted capacity work is not cancellable through a public process handle. The ledger tracks allocating/reserved/releasing extents, including pending extents in overlap checks. Snapshots read the original container's charged bytes; pending allocation metadata is not a second byte charge. `ScratchpadMemory` records a public allocation/release call as pending before its generator's first simulation step, rejects binding while that work or raw container requests remain pending, and requires the adapter's owner token while bound. Direct mutation of the container/private fields is outside the managed API. The adapter supplies capacity and ownership events only; buffer readiness metadata does not activate a service server or migrate DFG clients.
...
Part 7 wraps each selected `DMAEndpoint` or `NMCChannel` independently in `LegacyDMAAdapter`. Its `send`/`recv_message` return an observation process containing the unchanged native result object and a `LegacyDMALifecycle` record. Dual-side callers still post both partners; NMC receive still requires an explicit shape mode. Receive-side tail service and transmit-side local handoff project only facts present in the corresponding native result. Source-memory-read finish, addressed destination readiness and response-receipt timestamps remain unavailable, even if the legacy protocol internally waits for a response.
...
The public `predictor.predict.detect` and `embedding.hw_encoder.build_hardware_graph` now load their optional dependencies only after those guards. `HardwareEmbedding` is lazily exported from the same public module, with the same class/pickle location and model definition; its tensor implementation lives in `_hardware_embedding.py`. Guard behavior is tested through the actual entry points with tensor imports prohibited. Real encoder tensor/state-dict checks are separately conditional on installed Torch/PyG; no substitute inference is used.
