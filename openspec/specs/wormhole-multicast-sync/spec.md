# wormhole-multicast-sync Specification

## Purpose

Model finite shared rectangular multicast, addressed scalar synchronization and their composition with single-ASIC memory and compute workloads, with explicit resource costs, causal completion and fidelity limits.

## Contract scope

These requirements describe admitted runtime-configured version-one mixed
workloads: shared bounded rectangle transport, addressed scalar increments,
local threshold/data waits and optional finite FC/matmul stages. Legacy serial
projection inputs retain their narrower contract. The supported subset does
not imply arbitrary multicast destinations, general atomics, tensor reductions,
register-accurate reservations, numerical kernels or calibrated silicon timing.
Dimensions, clocks, widths, capacities and effective costs remain configurable
within the selected policy and hardware-binding restrictions.
