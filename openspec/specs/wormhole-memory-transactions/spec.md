# wormhole-memory-transactions Specification
...
MT-01..06 describe the addressed-memory and scalar-synchronization capability.
MT-D01..10 specify standalone memory replay version one, including its unsupported
operation checks and separate legacy DMA adapters. Compute and mixed workloads
compose admitted memory service through their own contracts; they do not make
standalone memory replay accept compute jobs or scalar operations. The supported
addressed scalar subset is specified in
[wormhole-multicast-sync](../wormhole-multicast-sync/spec.md). Aggregate service
and symbolic data readiness do not imply bank-accurate memory or tensor values.
