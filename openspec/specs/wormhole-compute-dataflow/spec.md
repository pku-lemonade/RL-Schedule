# wormhole-compute-dataflow Specification
...
CD-01..05 describe the compute/dataflow capability; CD-D01..10 specify the
finite FC/matmul workload and explicit legacy FC-chain adapter. General legacy
profile execution, numerical tensor evaluation and arbitrary operator lowering
remain outside these contracts. The mixed multicast/scalar composition in
[wormhole-multicast-sync](../wormhole-multicast-sync/spec.md) adds explicit modeled
notifications and local waits without relaxing standalone workload admission.
Hardware rates, clocks, storage and service policies remain declared inputs.
