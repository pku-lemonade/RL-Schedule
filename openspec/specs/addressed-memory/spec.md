...
`command_cycles`; data service SHALL then hold the mapped bank and channel
for `latency_cycles + ceil(bytes / channel_bytes_per_cycle)`. Acquisition
SHALL follow the registry's sorted order and release fully on completion,
