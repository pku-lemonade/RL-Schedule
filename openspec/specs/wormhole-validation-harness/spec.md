# wormhole-validation-harness Specification
...
VA-D01..11 specify version-one validation interfaces under the cross-child
[validation policy](../wormhole-validation/spec.md). Coverage and unsupported
features are scoped to selected adapters, cases and executed checks. Statements
about pending successor mechanisms describe the harness delivery boundary and
cases that lack those mechanisms; they do not suppress the separately delivered
mixed-runtime checks in
[wormhole-multicast-sync](../wormhole-multicast-sync/spec.md). Optional unavailable
consumer runs and missing external measurements retain their explicit limits.
