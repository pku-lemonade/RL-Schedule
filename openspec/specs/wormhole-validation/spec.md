# wormhole-validation Specification
...
These cross-child requirements govern evidence and delivery claims for the
finite single-ASIC model. The executable validation interfaces are specified in
[wormhole-validation-harness](../wormhole-validation-harness/spec.md), with mixed
multicast/scalar checks in
[wormhole-multicast-sync](../wormhole-multicast-sync/spec.md). A model-check pass,
specification synchronization or completed task checklist does not establish
functional-reference agreement, calibrated parameters or measured silicon timing.
The planning-only scenario in VA-07 applies when its stated premise holds.
