# instruction-programs Specification

## Purpose
Neutral instruction programs for the generic simulation layer: a batch may
declare named operation sequences bound to a sequencer execution unit.
Operations are the four existing transaction kinds; compilation expands
them into the unified immutable plan, and a deterministic sequencer issues
them in program order with explicit issue occupancy, dependency-gated
overlap, terminal-stop semantics and derived program-level results, without
touching program-free batches.
