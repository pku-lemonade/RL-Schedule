...
class DMAChannelBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if self.address.format != self.router.config.flit:
            raise ValueError("DMA address and router formats must match")
        if (self.address.mesh_x, self.address.mesh_y) != (self.router.x_dim, self.router.y_dim):
            raise ValueError("DMA address and router geometry must match")
        for link in (self.tx_link, self.rx_link):
            if link.flit_format != self.address.format:
                raise ValueError("DMA address and link formats must match")
...
class DMAReceiveResult:
    """One WDMA command completed at final payload-flit service."""
...
class DMAEndpoint:
    ...
    def __init__(
        ...
        *,
        aci_clock_mhz: float,
        flit_format: FlitConfig | None = None,
    ) -> None:
        ...
        self.env = env
        self.config = config
        self.flit_format = flit_format or FlitConfig()
        ...
        self.internal_datapath = Resource(env, capacity=config.internal_datapaths)
        if config.descriptor_issuers_per_fabric:
            self.descriptor_issuers = {
                fabric_id: Resource(env, capacity=1)
                for fabric_id in self.config.fabric_ids
            }
        else:
        ...
        self.tx_burst_arbiters = {
            fabric_id: RoundRobinArbiter(env) for fabric_id in self.config.fabric_ids
        }
    ...
    def bind_channel(self, binding: DMAChannelBinding) -> None:
        ...
        if address.fabric_id not in self.config.fabric_ids:
            raise ValueError("DMA binding uses an unconfigured fabric")
        if address.format != self.flit_format:
            raise ValueError("DMA endpoint and binding formats must match")
    ...
    def validate_channel_bindings(self) -> None:
        missing = set(self.config.fabric_ids) - self.bindings.keys()
        if missing:
            missing_names = ", ".join(
    ...
    def service_interval_aci_cycles(self) -> float:
        return (
            self.flit_format.physical_flit_bytes / self.service_bytes_per_aci_cycle
    ...
    def service_bytes_per_aci_cycle(self) -> float:
        ...
        raise RuntimeError(
            f"{self.node_type.name} requires an explicit port_bw for payload service"
        )
    ...
    def descriptor_issue_cycles(self) -> float:
        ...
        raise RuntimeError(
            f"{self.node_type.name} descriptor issue timing is not configured; "
    ...
    def max_outstanding_descriptors_per_channel(self) -> int:
        ...
        raise RuntimeError(
            f"{self.node_type.name} requires an explicit "
