...
| ✅ Done | `utils/definitions.py` | Added NodeType, FlitType, TransType enums; port constants (0-15 local, 100-103 direction); Flit class; helpers (direction_to_port, is_direction_port, is_local_port, compute_flit_count, port_to_direction); extended Message with src/dst_type, local ports, trans_type, broadcast, reduce, sync_mode; extended Event/TraceItem/TimeSlice with node_type/flit_count/is_dma/dmas list; removed FIXPATH (MULTICAST=1, BROADCAST=2). Commits `66af38f`, `6b3bfae`. |
...
| SEND task execution | Constructs Message, calls data_out.put(msg) | (1) determine dst_router and dst_local_port based on dst_type; (2) compute flit count; (3) wait for NMC channel idle; (4) wait for NMC startup latency; (5) allocate NMC SRAM port budget for payload bytes; (6) send via upload_link.put(msg) which triggers flit-level transmission |
...
| `SEND.execute` | Constructs Message(data=data, src=core.id, dst=child_core_id) | Construct Message with dst_type, dst_router (computed from dst: PE→core_id, GM→28+col_id, DDR→corner router), dst_local_port (PE=0, GM_WDMA_CH0=10 etc.), trans_type, channel assignment. |
