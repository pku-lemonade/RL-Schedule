...
from .utils.definitions import Message, NMCShapeMode
...
def _validate_dual_same_direction(
    ...
    _validate_common_streams(streams)
    if len({stream.source.fabric_id for stream in streams}) != 2:
...
def _validate_dual_full_duplex(
    ...
    _validate_common_streams(streams)
    for fabric_id in {stream.source.fabric_id for stream in streams}:
