...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Run the detailed simulator.")
    parser.add_argument("--arch", required=True, help="Architecture configuration JSON")
    parser.add_argument("--mapping", default="workloads/darknet19-4-4.json")
    parser.add_argument("--fail", default=DEFAULT_FAILURE_PATH)
    args = parser.parse_args()
    network = parse_mapping(args.mapping)
    mapper = NetworkMapper(network=network)
    mapper.gen_dfg()

    simulate(arch_path=args.arch,
             failure_path=args.fail,
             mapper=mapper,
             verbose=True)
