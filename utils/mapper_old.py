...
class NetworkMapper:
    ...
    def intersect(self, x: List[DimSlice], y: List[DimSlice]) -> bool | List[DimSlice]:
        res = []
        intersect_flag = True
    ...
    def toposort(self) -> List[int]:
        ...
        for node_id in range(1, self.node_counter+1):
            cur_node = self.dfg.get_node(node_id)
            node_in_degree[node_id] = len(cur_node.father)
        ...
        while not my_queue.empty():
            ...
            for son_id in cur_node.son:
                node_in_degree[son_id] -= 1
                if node_in_degree[son_id] == 0:
                    son_node = self.dfg.get_node(son_id)
                    my_queue.put(son_node)
