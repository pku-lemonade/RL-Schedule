...
class Arch:
    def __init__(
        ...
        self.noc = NoC(
            self.env, arch.noc, deterministic=deterministic, rank=rank
        ).build()
        ...
        if ncfg.enable_dma:
            self.nodes, self.gm, self.ddr = build_memory_system(
                ...
                arch.memory,
                arch.clock,
                include_interchip=ncfg.enable_interchip,
                include_all_interchip_port=ncfg.include_all_interchip_port,
                interchip_latency=ncfg.interchip_latency,
                shadow_cfg=arch.shadow,
                rank=rank,
                atomic_latency=atomic_latency,
            )
        elif ncfg.enable_interchip:
            self.nodes = attach_nodes(
                self.env,
                self.noc,
                include_dma=False,
                include_interchip=True,
                include_all_interchip_port=ncfg.include_all_interchip_port,
                interchip_latency=ncfg.interchip_latency,
                shadow_cfg=arch.shadow,
                rank=rank,
    ...
    def _build_cores(self, config, mapper, shadow_cfg=None):
        ...
        for cid in range(self.x_size * self.y_size):
            ...
            data_in, data_out = self.noc.attach_local(
                cid, self.config.noc.pe_local_port, cid
            )
