"""Standalone pipeline regression suite, also discoverable by unittest."""
...
def run_regressions():
    ...
    def launch(env, pipe, access, buf, chunk, work, rec, start=None, manual_wait=False):
        def p():
    ...
    def rdr():
        s = yield env.process(p.read(7, 0))
        rec.append(s)
    ...
    env.process(consume_alias())
    env.run()
    check(
        "T-H6.3 produce->consume triggers RAW (acquire=10)",
    ...
    def produce2():
        s = yield env.process(p.produce(1, 0))
        w.append({"acquire": env.now})
        yield env.timeout(2)
        s.release()

    env.process(produce2())
    env.run()
    check(
        "T-H6.3 produce->produce triggers WAW (second acquire=8)",
        any(r.get("acquire") == 8 for r in w if isinstance(r, dict)),
        f"{w}",
    )
    ...
    check(
        "T-H6.4 different chunks independent (R acquires 0)",
        r1["acquire"] == 0,
        f"{r1['acquire']}",
    )
    ...
    env.run()
    check(
        "T-H6.4 different buf_id independent",
    ...
    env.run()
    check("T-H6.4 two Pipeline instances isolated", rec2[0]["acquire"] == 0)
    ...
    check(
        "T-H6.5 RAW read acquires at write release (10)",
        r["acquire"] == w["release"] == 10,
        f"{r['acquire']} vs {w['release']}",
    )
    ...
    check(
        "T-H6.6 WAW second acquires at first release",
        w1["acquire"] == w0["release"] == 10,
        f"{w1['acquire']} vs {w0['release']}",
    )
    check("T-H6.6 WAW makespan 20", max(r["release"] for r in rec) == 20)
    check("T-H6.6 write intervals do not overlap", w0["release"] <= w1["acquire"])
    ...
    check(
        "T-H6.7 WAR write acquires at read release (10)",
        w["acquire"] == r["release"] == 10,
        f"{w['acquire']} vs {r['release']}",
    )
    ...
    check("T-H6.8 R-R both acquire at 0", r0["acquire"] == r1["acquire"] == 0)
    check("T-H6.8 R-R overlap (both release 10)", r0["release"] == r1["release"] == 10)
    st = p._states[(0, 0)]
    check(
        "T-H6.8 read_count returns to 0 after both release",
    ...
    w = next(x for x in rec if x["access"] == Access.WRITE)
    check(
        "T-H6.9 writer acquires at max reader release (10)",
    ...
    env.run()
    check(
        "T-H6.11 ready path adds 0 cycles (acquire=20)",
        ready_acq == [20],
        f"{ready_acq}",
    )
    ...
    env.run()
    check(
        "T-H6.11 work=0 acquire==release timestamp",
    ...
    env = simpy.Environment()
    p = Pipeline(env, auto_consume=False)
    rec = []
    launch(env, p, Access.WRITE, 0, 0, 10, rec)
    launch(env, p, Access.READ, 0, 0, 5, rec)
    env.run()
    ...
    r = next(x for x in rec if x["access"] == Access.READ)
    check(
        "T-H6.12 manual W/R both register/acquire at 0 (no stall)",
        w["acquire"] == r["acquire"] == 0,
        f"{w['acquire']},{r['acquire']}",
    )
    check(
        "T-H6.12 manual write/read logged no auto acquire event",
    ...
    r = next(x for x in rec if x["access"] == Access.READ)
    check(
        "T-H6.13 manual wait READ blocks until write release (10)",
        r["acquire"] == w["release"] == 10,
        f"{r['acquire']}",
    )
    check(
        "T-H6.13 wait logged one acquire event",
    ...
    w = next(x for x in rec if x["access"] == Access.WRITE)
    check(
        "T-H6.14 manual wait WRITE blocks for readers (6)",
        w["acquire"] == r["release"] == 6,
        f"{w['acquire']}",
    )
    ...
    env.run()
    check(
        "T-H6.14 second manual writer without wait does not block",
        rec[1]["acquire"] == 0,
    )
    ...
    check("T-H6.16 register/acquire/release labels present", reg and acq and rel)
    times = [e["time"] for e in p.events]
    check("T-H6.16 timestamps non-decreasing", times == sorted(times), f"{times}")
    check(
        "T-H6.16 hazard invariant READ.acquire>=WRITE.release",
        acq["time"] >= rel["time"],
    )
    ...
    env.run()
    writes = sorted(
        (x for x in rec if x["access"] == Access.WRITE), key=lambda x: x["acquire"]
    )
    no_overlap = all(
        writes[i]["release"] <= writes[i + 1]["acquire"] for i in range(len(writes) - 1)
    )
    check("T-H6.18 32 writers serialize with no overlap", no_overlap)
    check(
        "T-H6.18 makespan == sum of work (no overlap/starvation)",
    ...
    env.run()
    reads = sorted(
        [x for x in rec if x["access"] == Access.READ], key=lambda x: x["acquire"]
    )
    writes = sorted(
        [x for x in rec if x["access"] == Access.WRITE], key=lambda x: x["acquire"]
    )
    ...
    r1, r2 = reads
    check(
        "T-H6.19 W1 waits for R1 (acq=4)",
        w1["acquire"] == 4 and w1["release"] == 5,
        f"{w1['acquire']},{w1['release']}",
    )
    check(
        "T-H6.19 R2 waits for W1 (acq=5)",
        r2["acquire"] == 5 and r2["release"] == 15,
        f"{r2['acquire']},{r2['release']}",
    )
    check(
        "T-H6.19 W2 waits for BOTH W1(5) and R2(15) -> acq=15",
    ...
    check(
        "T-H6.20 late R2 waits for W (acq=15)",
        r2["acquire"] == w["release"] == 15,
        f"{r2['acquire']} vs {w['release']}",
    )
    ...
    p = Pipeline(env)
    check(
        "T-H6.21 wait READ on fresh buffer returns at 0", run_wait(p, Access.READ) == 0
    )
    check(
        "T-H6.21 wait logged only an acquire (no register)",
    ...
    check(
        "T-H6.22 single-buffer download->compute D+C=10",
        single_mk == 10,
        f"{single_mk}",
    )
    check(
        "T-H6.22 double-buffer < 2*(D+C)=20 (overlap)", double_mk < 20, f"{double_mk}"
    )
    ...
    env.run()
    check(
        "T-H6.22 blocked hazard waiter holds no shadow slot (level=1 of 2)",
        probe == [1],
        f"{probe}",
    )
        ...
        def update(self, src_node, dst_node):
            ...
            if src_node.operation in (OperatorType.LOAD_FEAT, OperatorType.RECV):
                dst_node.received_input += Slice(
                    tensor_slice=src_node.input_size
                ).size()
            elif src_node.operation in (OperatorType.STORE, OperatorType.SEND):
                dst_node.received_input += Slice(
                    tensor_slice=src_node.output_size
                ).size()
            if dst_node.operation in self._comp:
                if (
                    dst_node.received_input
                    != Slice(tensor_slice=dst_node.input_size).size()
                ):
                    return
                dst_node.ready = True
    ...
    dfg.add_node(
        2, OperatorType.POOL, 0, input_size=ds(4, 4, 4, 4), output_size=ds(4, 4, 4, 4)
    )
    dfg.add_node(3, OperatorType.STORE, 0, output_size=ds(4, 4, 4, 4))
    dfg.add_edge(1, 2)
    dfg.add_edge(2, 3)
    cfg = ArchConfig(
        core=CoreConfig.model_construct(spm=SPMConfig(size=10**9)), noc=NoCConfig()
    )
    cfg.nodes.enable_dma = False
    cfg.nodes.enable_interchip = False
    ...
    r = next(x for x in rec if x["access"] == Access.READ)
    check(
        "T-H6.24 IntEnum buf_id equals int key (RAW blocks at 10)",
        r["acquire"] == 10,
        f"{r['acquire']}",
    )
    env = simpy.Environment()
    ...
    env.run()
    check(
        "T-H6.24 distinct enum values independent",
    ...
    assert failed == 0, f"{failed} pipeline regressions failed"


class PipelineTests(unittest.TestCase):
    def test_pipeline_regressions(self):
