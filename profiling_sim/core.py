        ...
        for son_id in task.successors:
            ...
            if son_node.ready and not son_node.executed:
                ...
                if son_node.core_id == self.id:
                    ...
                    heapq.heappush(
                        self.cores[son_node.core_id].scheduler.ready_tasks, new_task
                    )

    def schedule(self):
        total = (
            len(self.ready_tasks)
            + len(self.ready_comp_tasks)
            + len(self.ready_comm_tasks)
            + len(self.ready_io_tasks)
        )
        if total == 0:
            return [], [], None
...
class Core:
    def __init__(
        ...
        self.sram_read = simpy.Resource(env, capacity=rp if rp and rp > 0 else 10**6)
        self.sram_write = simpy.Resource(env, capacity=wp if wp and wp > 0 else 10**6)
        self.events = []
        self.index2id = {}
    ...
    def _build_shadow_pipelines(self):
        ...
        self.matrix_pipeline = ShadowPipeline(
            self.env,
            f"core{self.id}.matrix",
            [f"matrix.{index}" for index in range(len(cfg.matrix))],
            list(cfg.matrix),
            occupancy=d,
            ii=cfg.ii,
        )
        self.vector_pipeline = ShadowPipeline(
            self.env,
            f"core{self.id}.vector",
            [f"vector.{index}" for index in range(len(cfg.vector))],
            list(cfg.vector),
            occupancy=d,
            ii=cfg.ii,
        )
        for ch in range(self.nmc.channel_store.capacity):
            self.sramc_pipelines[(ch, True)] = ShadowPipeline(
                self.env,
                f"core{self.id}.sramc_upld{ch}",
                [sramc_entry(ch, True)],
                [cfg.sramc_upld],
                occupancy=d,
                ii=cfg.ii,
            ...
            self.sramc_pipelines[(ch, False)] = ShadowPipeline(
                self.env,
                f"core{self.id}.sramc_dnld{ch}",
                [sramc_entry(ch, False)],
                [cfg.sramc_dnld],
                occupancy=d,
                ii=cfg.ii,
            )
    ...
    def enter_matrix(self, tag=None, cal_cycles: int = 0):
        return self.env.process(
            self.matrix_pipeline.enter(
                tag,
                {
                    self.matrix_pipeline.entry_ids[
                        self.shadow_cfg.matrix_compute_stage
                    ]: cal_cycles
                },
            )
        )
    ...
    def enter_vector(self, tag=None, cal_cycles: int = 0):
        return self.env.process(
            self.vector_pipeline.enter(
                tag,
                {
                    self.vector_pipeline.entry_ids[
                        self.shadow_cfg.vector_compute_stage
                    ]: cal_cycles
                },
            )
        )
    ...
    def enter_sramc(self, channel: int, upload: bool, tag=None):
        return self.env.process(self.sramc_pipelines[(channel, upload)].enter(tag))
    ...
    def execute(self):
        ...
        while True:
            ...
            for task, tag in pending:
                logger.info(
                    f"Time {self.env.now:.2f}: {tag}Task {task.index} at Core {self.id}"
                )
                self.events.append(
                    Event(
                        type=task.operation,
            ...
            for event in list(self.running_event):
                if event.triggered:
                    ...
                    logger.info(
                        f"Time {self.env.now:.2f}: Task {task.index} finished at Core {self.id}"
                    )
                    self.events[self.index2id[task.index]].end_time = int(
                        round(self.env.now)
                    )
                    ...
                    if task.index in self.executed_task:
                        raise RuntimeError(
                            f"Repetitive execution: Core {self.id} task {task.index}"
                        )
