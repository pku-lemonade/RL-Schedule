...
class Link:
    def __init__(
        ...
        self.para_dist = NoCDist(
            shape=self.bandwidth * 0.5, rate=0.5, deterministic=deterministic
        )
    ...
    def _payload_chunks(self, msg):
        ...
        quantum = msg.burst_len_mode or self.config.burst_beats
        if quantum < 0:
            raise ValueError("burst quantum must be nonnegative")
        if quantum == 0:
            return [payload]
        beat_bytes = quantum * self.bandwidth
        chunks = []
        rem = payload
        while rem > 0:
    ...
    def calc_latency(self, msg):
        ...
        if msg.transfer_mode == TransferMode.SINGLE_SIDE and not msg.is_control:
            var_bw = var_bw * self.config.single_side_bandwidth_factor
...
class Router:
    ...
    def _delivery_port(self):
        return self.config.pe_local_port
    ...
    def routing(self, msg, from_local: bool, in_port=None):
        ...
        if msg.trans_type in (TransType.MULTICAST, TransType.BROADCAST):
            ...
            if mask:
                ...
                logger.warning("router %s received multicast with empty mask", self.id)
            return
        ...
        if cross:
            if msg.sync and not msg.is_control:
                raise ProfilingSimError(
                    f"cross-chip outer sync is unsupported (dst_rank={msg.dst_rank})"
                )
            if msg.dst_rank not in self.egress_table:
                raise ProfilingSimError(
                    f"no egress InterChip for remote rank {msg.dst_rank} "
                    f"on chip {self.chip_rank}"
                )
            target, _eg_port = self.egress_table[msg.dst_rank]
        single = (
            msg.transfer_mode == TransferMode.SINGLE_SIDE
        ...
        if single and from_local:
            if (
                0 <= msg.src < self.dim_x * self.dim_y
                and 0 <= msg.dst < self.dim_x * self.dim_y
            ):
        ...
        try:
            if target == self.id:
                delivery_port = (
                    self.egress_table[msg.dst_rank][1] if cross else msg.dst_local_port
                )
                ...
                if single:
                    ...
                    if src_router != self.id:
                        ...
                        if back is not None:
                            back.put(
                                self._control_msg(msg, msg.src, msg.dst, req=False)
                            )
                if do_sync:
                    src_router = self.resolve_router(msg.src)
                    if src_router != self.id:
                        ...
                        if acq_out is not None:
                            acq_out.put(
                                self._sync_msg(msg, msg.src, msg.dst, to_dst=False)
                            )
            else:
    ...
    def _fixed_path_next(self, msg):
        ...
        try:
            ...
            raise ProfilingSimError(f"router {self.id} not on FIXPATH {path}") from exc
        if idx + 1 >= len(path):
            raise ProfilingSimError(
    ...
    def _control_msg(self, msg, dst, src, req: bool):
        ...
        return msg.model_copy(
            update={
                "dst": dst,
                "src": src,
                "data": [DimSlice(start=0, end=self.config.control_bytes)],
                "element_bytes": 1,
                "header_bytes": 0,
    ...
    def _sync_msg(self, msg, dst, src, to_dst: bool):
        from .definitions import DimSlice
        ...
        return msg.model_copy(
            update={
                "dst": dst,
                "src": src,
                "data": [DimSlice(start=0, end=self.config.sync_bytes)],
                "element_bytes": 1,
                "header_bytes": 0,
                "transfer_mode": TransferMode.DUAL_SIDE,
                "is_control": True,
                ...
                "trans_type": TransType.SINGLECAST,
                "dst_local_port": (
                    msg.dst_local_port if to_dst else msg.src_local_port
                ),
                "priority": 0,
                "burst_len_mode": 0,
                "fifo_hw_id": -1,
    ...
    def _fifo_return(self, msg):
        if (
            ...
            if fifo["credits"].level < fifo["depth"]:
                ...
                logger.debug("FIFO %s credit return dropped (already full)", key)
        ...
        if mask & (1 << self.id):
            ...
            if msg.sync and not msg.is_control:
                ...
                if src_router != self.id:
                    ...
                    acq_out = self.links[acq_dir]["out"]
                    if acq_out is not None:
                        acq_out.put(self._sync_msg(msg, msg.src, msg.dst, to_dst=False))
        for direction, submask in self._multicast_branches(mask):
            if submask:
                out = self.links[direction]["out"]
                if out is not None:
                    replica = msg.model_copy(
                        update={"dst_mask": submask, "dst": self._neighbor(direction)}
                    )
                    out.put(replica)
                    delivered = True
        if not delivered:
    ...
    def _reduce_run(self, task_id):
        ...
        if node["is_root"]:
            yield self.env.process(self.route_local(merged, merged.dst_local_port))
        else:
            yield self.env.timeout(self.per_hop_time)
...
class NoC:
    def __init__(self, env, config, deterministic: bool = False, rank: int = 0):
        ...
        self.y = config.y
        self.rank = rank
        from .nodes import NodeLayout

        self.layout = NodeLayout(config)
        self.router_config = config.router.model_copy(
            update={"pe_local_port": config.pe_local_port}
        )
    ...
    def build(self):
        ...
        for rid in range(self.x * self.y):
            ...
            r = Router(
                self.env, self.router_config, rid, self.x, self.y, local_inj_cap=cap
            )
        ...
        for y in range(self.y):
            for x in range(self.x):
                ...
                if y < self.y - 1:
                    self._connect(rid, Direction.NORTH, rid + self.x, Direction.SOUTH)
    ...
    def attach_local(self, router_id: int, port: int, node_id: Optional[int] = None):
        if not 0 <= router_id < len(self.routers):
            raise ValueError("local attachment router is outside the mesh")
        if port < 0 or port in self.routers[router_id].local_ports:
            raise ValueError("local attachment port is invalid or already bound")
        if node_id is None:
            node_id = router_id
        if node_id in self.node_router and self.node_router[node_id] != router_id:
            raise ValueError("node already belongs to another router")
        bb = self.router_config.burst_bubble
        node_to_router = Link(
            self.env,
            self.link_config,
        ...
        self.routers[router_id].bind_local_port(port, node_to_router, router_to_node)
        if node_id >= 0:
    ...
    def register_fifo(self, hw_id: int, logic_id: int, depth: int):
        if not 0 <= hw_id < self.config.fifo_hardware_count:
            raise ValueError("FIFO hardware ID is outside configured count")
        if not 0 <= logic_id < self.config.fifo_logical_count:
            raise ValueError("FIFO logical ID is outside configured count")
        if depth < 1:
