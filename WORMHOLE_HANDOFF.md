...
- Active follow-on change: **`wormhole-external-validation`**, **22/35 tasks** at the Part 7 offline checkpoint. Six validated local commits (`c4884ca`, `db13ecb`, `cecaa96`, `56bf613`, `aa11a7f`, `c5e9cb8`) implement strict external contracts, typed intervals, portable capture kits, the Wormhole collector, paired campaign orchestration and sealed calibration. The Part 7 checkpoint adds portable raw-reference packaging and an independent lineage/equivalence audit; obtain its commit with `git log -1 --oneline -- openspec/changes/wormhole-external-validation/delivery.md`. See [delivery.md](openspec/changes/wormhole-external-validation/delivery.md), [progress.md](openspec/changes/wormhole-external-validation/progress.md), [tasks.md](openspec/changes/wormhole-external-validation/tasks.md) and [exact identities](openspec/changes/wormhole-external-validation/delivery-identities.json).
- The active external change cannot complete its remaining 13 tasks on this host: `/dev/tenstorrent`, `tt-smi`, a ttsim shared library and a pinned TT-Metal checkout/build are absent. Offline mechanics and compatibility are verified, but no synthetic fixture is classified as actual ttsim, silicon-timing, measured-calibration or held-out evidence. Resume by supplying clean capture bundles from compatible pinned workers; re-admit and audit them before checking any remaining task. Do not archive the change at this checkpoint.
