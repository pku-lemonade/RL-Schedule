...
Prepared on 2026-09-17 for continuing this work on another machine; delivery status updated on 2026-09-20. Paths in this document are relative to the repository root; the old local checkout path is not a runtime requirement.
...
```text
Continue the Wormhole simulator work in this RL-Schedule checkout on feiyang-dev. Read WORMHOLE_HANDOFF.md and verification.md, evidence-map.json, reconciliation.md and the planning artifacts in openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/. All seven children are delivered (233/233 tasks), including validation and multicast at 35/35 each. The umbrella is complete at 38/38: the evidence audit reviews all 27 parent requirements / 30 scenarios, and task 8.3 delivers seven canonical specs containing all 100 distinct parent/child requirements and 178 scenarios. Read spec-sync-identities.json for exact canonical hashes. Use OpenSpec 1.11.0 through the documented pinned npm invocation and explicit --type change/spec validation flags. All eight completed changes are archived; no original implementation or specification-closure tasks remain. External evidence and optional ML follow-up are separate scopes. Use the archive record to resolve immutable historical evidence paths. Do not repeat completed implementation or equate planning status with delivery. Test, type-check, lint and commit each completed part before proceeding. Preserve compatibility, configurable hardware parameters and honest evidence tiers. Respond in English first, then Chinese. Do not archive implicitly, repeat history cleanup, or push unless requested.
```
