...
Prepared on 2026-09-17 for continuing this work on another machine; delivery status updated on 2026-09-21. Paths in this document are relative to the repository root; the old local checkout path is not a runtime requirement.
...
- Active follow-on change: **`wormhole-external-validation`**, **25/35 tasks** after completing real ttsim functional collection and the offline Part 7 checks. Eleven validated local commits through `ae4bedf` plus the current profiler-enabled checkpoint implement strict external contracts, typed intervals, portable capture kits, the Wormhole collector, paired campaign orchestration, sealed calibration, independent evidence audit, actual pinned ttsim evidence and the exact pinned profiler CSV dialect. See [delivery.md](openspec/changes/wormhole-external-validation/delivery.md), [progress.md](openspec/changes/wormhole-external-validation/progress.md), [tasks.md](openspec/changes/wormhole-external-validation/tasks.md) and [exact identities](openspec/changes/wormhole-external-validation/delivery-identities.json).
- The active external change is complete for this host at 25/35; its remaining 10 tasks are explicitly deferred because `/dev/tenstorrent` is absent, `tt-smi` is unavailable and no named Wormhole board, firmware inventory or live profiler output was supplied. The pinned worker now has a fixed 25-argument hardware mode, explicit device binding, a sealed plan, exact same-RISC profiler selection, typed output manifest and a Tracy-enabled runtime whose dependencies are hashed in `bin/runtime`. A direct invocation validated that contract and reached `MeshDevice::create_unit_mesh`, then failed with `No chips detected in the cluster`. Two clean profiler-enabled ttsim kits independently reproduced all three committed functional captures and references byte for byte. Resume on a compatible named Wormhole worker by materializing the build artifacts declared in the campaign and calling `collect_wormhole_capture`; re-admit and audit the returned functional record, profiler CSV and manifest before checking any remaining task. The deferred boxes remain unchecked, and the change is not archived or pushed.
- Latest completed child: **`wormhole-multicast-sync`**, **35/35 tasks**, **12 requirements and 31 acceptance scenarios**. Shared bounded tree/unicast transport, canonical memory and scalar service, local waits, attached compute, retained resume, real assumed-profile fixtures and independent CLI/harness audits are delivered. See [delivery.md](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/delivery.md), [progress.md](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/progress.md), [verified audit](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/implementation-audit.md) and [exact identities](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/delivery-identities.json). Part 6 is `31f9526`; the final multicast delivery is `f6ca63b`. Earlier serial-prototype completion claims were audited and repaired, without reducing the approved scope.
- **`wormhole-single-chip-simulation-plan` is complete at 38/38 milestones**, with all seven children delivered (233/233 tasks). The [verification report](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/verification.md) and [evidence map](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/evidence-map.json) review 27 parent requirements and 30 parent scenarios. Task 8.3 has now merged all 12 parent/child deltas into **seven canonical main specifications, 100 unique requirement IDs and 178 scenarios**; see the [reconciliation delivery](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/reconciliation.md) and [canonical identities](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/spec-sync-identities.json). No implementation or specification-closure milestone remains. The eight completed changes were archived on 2026-09-20; see the [archive record](openspec/changes/archive/2026-09-20-wormhole-archive.md). External timing/functional evidence and optional ML remain unvalidated.
...
6. Follow the approved sequence: explore a child using the preceding child's findings, create/validate its concrete change, apply and validate/commit its parts, then explore the next child. All seven children, the umbrella evidence audit and specification reconciliation are complete. Select any further implementation or external-validation work through its own explicit scope.
...
8. Do not sync/archive the umbrella prematurely or mark it delivered merely because its planning files exist. The parent/child evidence map and seven canonical main specs are now reconciled. Archival is complete with the synchronized main specs preserved. Historical evidence manifests keep original logical paths; use the archive record to resolve them.

## Completed work and remaining roadmap

The umbrella is [wormhole-single-chip-simulation-plan](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/tasks.md). Its evidence audit and canonical specification reconciliation complete **38/38 milestones**. The seven children total **233/233 completed tasks**. Likewise, OpenSpec `status` can say `isComplete: true` for complete planning artifacts; use `instructions apply` and task boxes to inspect implementation progress.
...
| Umbrella audit / closure | 38/38 | All parent evidence reviewed; seven canonical specs retain 100 distinct parent/child requirements and 178 scenarios; all eight changes archived |
...
1. The completed multicast child's [delivery](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/delivery.md), [identities](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/delivery-identities.json), [proposal](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/proposal.md), [design](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/design.md), [requirements](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/specs/wormhole-multicast-sync/spec.md), [tasks](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/tasks.md) and [implementation audit](openspec/changes/archive/2026-09-20-wormhole-multicast-sync/implementation-audit.md).
2. The preceding compute child's [delivery report](openspec/changes/archive/2026-09-20-wormhole-compute-dataflow/delivery.md) and [progress evidence](openspec/changes/archive/2026-09-20-wormhole-compute-dataflow/progress.md), including independent oracles, exact identities, compatibility checks and known limitations.
3. The umbrella [design](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/design.md) and [VA-01..07 validation requirements](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/specs/wormhole-validation/spec.md).
...
Unsupported or unvalidated: general numerical tensor execution in the abstract simulator, TT-Metal/RISC-V/ISA simulation, detailed unpack/math/pack engines, automatic tilize/conversion/general operator lowering, split-K/reductions, arbitrary cross-worker notifications, exact NIU/DRAM-bank/channel/cache behavior, general multicast/atomics/semaphores beyond the scoped child policies, dynamic kernels, interchip/host/PCIe modeling, new predictor/RL models and calibrated silicon timing. The external change has run its three bounded TT-Metal producers on pinned ttsim for functional evidence; no physical hardware measurement has been performed.
...
Task 8.3 delivers seven main specs with all 100 requirement bodies and 178 scenarios preserved, explicit adapter/milestone scope notes and no duplicate IDs or delta-operation headings. All seven main specs and eight related changes pass strict validation. Reapplying the reviewed merge changes zero files. The 12 identity tests pass; strict Pyright reports zero errors/warnings, and the recorded 45-path Ruff scope passes. Exact runtime/asset hashes are unchanged, so the prior full-suite result remains separately identified. The [reconciliation delivery](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/reconciliation.md) records the checks and [spec-sync-identities.json](openspec/changes/archive/2026-09-20-wormhole-single-chip-simulation-plan/spec-sync-identities.json) records canonical hashes.

The destination global CLI is OpenSpec 1.3.1, which lacks the path metadata required by the generated synchronization skill. Use the pinned `npm exec --yes --package=@fission-ai/openspec@1.11.0 -- openspec ...` invocation, verified under Node 22.22.2, or an installed 1.11.0 CLI. This part used the npm cache and did not replace the global CLI or project dependencies. The eight completed changes were archived with `--skip-specs` after strict validation and exact delta-to-main comparison. Validate current main specs with `--specs`; inspect archived task checklists directly. Use explicit `--type change` or `--type spec` when names overlap.

## Completed multicast child: current execution evidence

The final detailed suite reports **557 discovered, 556 passed, zero failures/errors,
one optional Torch/PyG skip** (189.838 s). Strict Pyright, changed/predecessor scoped
Ruff, strict OpenSpec and whitespace checks pass. Five new executable multicast
fixtures and three bounded suites pass; the original 19-case validation suite
still passes 123 model checks with optional ML blocked. Ten predecessor replay
examples retain their versions/timings/exits. The actual root Darknet19 smoke
completes 37,888 nodes on 16 cores/48 links with 11 JSON-round-tripped windows.
Historical root local-remap failures remain separately recorded, not claimed fixed.

Use `replay_multicast_sync --workload` with `mixed_two_rounds.json` or
`wormhole_b0_mixed_assumed.json`; the old synthetic-ID/profile fixture remains
invalid. See [multicast usage](simulator_detailed/docs/multicast_sync.md). The
executable result includes live ownership, scalar values and slot generations.
Hardware dimensions, clocks, widths/capacities and costs remain declared inputs.
Functional-reference and silicon-timing evidence remain unvalidated. No numerical
tensors, general atomics, arbitrary multicast trees or dynamic kernels are implied.

## Completed validation child: implementation record

The existing [35-task checklist](openspec/changes/archive/2026-09-20-wormhole-validation-harness/tasks.md) is complete. Each numbered part ended with validation, a progress record and a commit. Keep this sequence as the delivered record; do not repeat its implementation.
...
- ttsim is a potential functional source, not a timing oracle. There is no assumed universal ttsim trace exporter and no automatic vendor-tool launcher in this child. Pinned design source identities are already recorded in the archived validation design.
...
The multicast child supplies the scoped tree/scalar/compute mechanisms, and the umbrella audit and specification reconciliation are complete. Preserve each child's delivery and evidence boundaries when selecting any subsequent work.
...
Inspect active changes and validate the delivered main specifications (completed changes are archived):

```sh
npm exec --yes --package=@fission-ai/openspec@1.11.0 -- openspec list --json
...
- No optional model forward/inference/RL training or matched hardware timing measurement was performed. Pinned ttsim functional runs are committed separately under the active external-validation change; they do not establish silicon timing. Missing tools/imports are unavailable checks, not successes.

## Short prompt for the next agent

```text
Continue `wormhole-external-validation` in this RL-Schedule checkout on feiyang-dev. Read WORMHOLE_HANDOFF.md and the proposal, design, specs, tasks, progress and delivery identities in openspec/changes/wormhole-external-validation/. Progress is 25/35. The profiler-enabled pinned worker and portable runtime manifest are implemented and verified through sealed-plan admission and device discovery; this host then reports `No chips detected in the cluster`. The committed ttsim matrix has been reproduced twice from clean kits. Complete only the 10 unchecked hardware-dependent tasks using a named Wormhole B0 worker: materialize every campaign-declared build artifact, run `collect_wormhole_capture` for all three cases, import and audit the functional records/profiler CSV/manifests, run the paired comparisons and disjoint fit/held-out evaluation, then perform the final clean matrix. Do not mark a task complete from unavailable or ttsim-only evidence. Use OpenSpec 1.11.0 through the documented pinned npm invocation. Test, type-check, lint and commit each completed part. Preserve compatibility, configurable hardware parameters and honest evidence tiers. Respond in English first, then Chinese. Do not archive or push unless requested.
```
